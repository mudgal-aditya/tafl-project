function parseGrammar(input) {
  let rules = {};
  input.split("\n").forEach(line => {
    if (!line.trim()) return;
    let [left, right] = line.split("->").map(s => s.trim());
    rules[left] = right.split("|").map(r => r.trim());
  });
  return rules;
}

function printGrammar(g) {
  let out = "";
  for (let k in g) {
    out += `${k} -> ${g[k].join(" | ")}\n`;
  }
  return out;
}

/* STEP 1: REMOVE USELESS SYMBOLS */
function removeUseless(grammar) {
  let generating = new Set();

  let changed = true;
  while (changed) {
    changed = false;
    for (let A in grammar) {
      for (let prod of grammar[A]) {
        if ([...prod].every(c => !grammar[c] || generating.has(c))) {
          if (!generating.has(A)) {
            generating.add(A);
            changed = true;
          }
        }
      }
    }
  }

  let filtered = {};
  for (let A in grammar) {
    if (generating.has(A)) {
      filtered[A] = grammar[A].filter(p =>
        [...p].every(c => !grammar[c] || generating.has(c))
      );
    }
  }

  return filtered;
}

/* STEP 2: REMOVE NULL PRODUCTIONS */
function removeNull(grammar) {
  let nullable = new Set();

  let changed = true;
  while (changed) {
    changed = false;
    for (let A in grammar) {
      for (let prod of grammar[A]) {
        if (prod === "ε" || [...prod].every(c => nullable.has(c))) {
          if (!nullable.has(A)) {
            nullable.add(A);
            changed = true;
          }
        }
      }
    }
  }

  let newGrammar = {};

  for (let A in grammar) {
    newGrammar[A] = new Set();

    for (let prod of grammar[A]) {
      if (prod === "ε") continue;

      let combos = generateCombinations(prod, nullable);
      combos.forEach(c => newGrammar[A].add(c || "ε"));
    }
  }

  for (let A in newGrammar) {
    newGrammar[A] = [...newGrammar[A]];
  }

  return newGrammar;
}

function generateCombinations(prod, nullable) {
  let results = [""];
  for (let c of prod) {
    let temp = [];
    for (let r of results) {
      temp.push(r + c);
      if (nullable.has(c)) temp.push(r);
    }
    results = temp;
  }
  return [...new Set(results)];
}

/* STEP 3: REMOVE UNIT PRODUCTIONS */
function removeUnit(grammar) {
  let unitPairs = {};

  for (let A in grammar) {
    unitPairs[A] = new Set([A]);
  }

  let changed = true;
  while (changed) {
    changed = false;
    for (let A in grammar) {
      for (let prod of grammar[A]) {
        if (grammar[prod]) {
          for (let x of unitPairs[prod]) {
            if (!unitPairs[A].has(x)) {
              unitPairs[A].add(x);
              changed = true;
            }
          }
        }
      }
    }
  }

  let newGrammar = {};

  for (let A in grammar) {
    newGrammar[A] = new Set();

    for (let B of unitPairs[A]) {
      for (let prod of grammar[B]) {
        if (!grammar[prod]) {
          newGrammar[A].add(prod);
        }
      }
    }
  }

  for (let A in newGrammar) {
    newGrammar[A] = [...newGrammar[A]];
  }

  return newGrammar;
}

/* BUTTON HANDLERS */

function runStep1() {
  let g = parseGrammar(document.getElementById("grammarInput").value);
  let result = removeUseless(g);
  document.getElementById("output").innerText =
    "Step 1: Remove Useless Symbols\n\n" + printGrammar(result);
}

function runStep2() {
  let g = parseGrammar(document.getElementById("grammarInput").value);
  let result = removeNull(g);
  document.getElementById("output").innerText =
    "Step 2: Remove Null Productions\n\n" + printGrammar(result);
}

function runStep3() {
  let g = parseGrammar(document.getElementById("grammarInput").value);
  let result = removeUnit(g);
  document.getElementById("output").innerText =
    "Step 3: Remove Unit Productions\n\n" + printGrammar(result);
}